...
import { ICON_ARROW_LEFT_SVG, ICON_SPEAKER_BLACK } from '../../../constaints/icons';
...
export const SelectAccountModal = ({
  ...
  const renderItem = ({ item }: { item: ChannelItem | ExternalAccount | any }) => {
    return (
      <TouchableOpacity activeOpacity={1} onPress={() => setSelectedChannel(item)}>
        <Box width={width - 48} position="relative" left={24} marginBottom={16} height={72}>
              ...
              >
                <Box width="90%" height="100%" direction="row" alignItems="center">
                  ...
                  <Box flex={1} height="100%" paddingLeft={16}>
                    <Box height={23}>
                      <Text color="rgba(255, 255, 255, 1)" fontWeight={400} fontSize={14}>
                        {item?.title}
                      </Text>
                    </Box>
      ...
      ListFooterComponent={
        <>
          {!isLoading && !isFetching && !data?.data?.length ? (
            <Box width="100%" marginTop={32} alignItems="center" justifyContent="center">
