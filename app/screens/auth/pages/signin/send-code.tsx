...
export const SendCode = ({
  ...
  const { getPhoneNumber, getExpireTime, setToken } = useContext(signupContext);
  ...
  const submitOtpCodeHandler = async (data: any) => {
    ...
    const { isError, isSuccess, res } = await submitCodeApiHandler(phoneInputValue, otp_code);
            ...
            <Controller
              ...
              render={({ field: { onChange, onBlur, value } }) => (
                <Input
                  ...
                  value={value}
                  hasError={errors?.['otp_code'] ? true : false}
                  additionalProps={{
                    inputMode: 'numeric',
                  }}
                />
              ...
              rules={validators}
            />
            <Box direction="row" alignItems="center" justifyContent="space-between" paddingTop={16}>
              <TouchableOpacity activeOpacity={1} onPress={goBackToInsertPhoneWindow}>
                <Text
