...
export const InsertPhone = ({ setCurrentWindowHandler, navigateToLoginPage }: any) => {
  const {
    control,
    setError,
    clearErrors,
  ...
  const onSubmit = async (data: any) => {
    ...
    const { res, isError, isSuccess } = await requestCodeApiHandler(formattedPhoneNumber);
  ...
  return (
    <Box flex={1} alignItems="center">
      ...
      <Box position="absolute" bottom={10} width="100%" alignItems="center">
        <TouchableOpacity activeOpacity={1} onPress={navigateToLoginPage}>
          <Text
