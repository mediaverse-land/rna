...
import { ICONS_GOOGLE_BLUE, ICON_STREAM, ICON_TRASHBEEN } from '../../../constaints/icons';
      ...
      <TouchableOpacity activeOpacity={1}>
        <Box width={'100%'} height={56} marginBottom={16} borderRadius={16}>
          ...
          >
            ...
            <Box direction="row" alignItems="center">
              <TouchableOpacity activeOpacity={1} onPress={() => removaAccountHandler(item)}>
                <ICON_TRASHBEEN />
              </TouchableOpacity>
            </Box>
          </Box>
  ...
  return (
    <Box marginTop={48} paddingRight={24} paddingLeft={24}>
      <FlatList
        ...
        keyExtractor={_key}
        ListFooterComponent={<Box height={52}>{isLoading ? <LoadingSpinner /> : null}</Box>}
      />
    </Box>
  );
};
