...
export function ImagePageMasonryList() {
    ...
    const renderMasonryList = masonryListImagesMockData.map((list: { row: ImageType[] }, index: number) => {
        const findTwoInTwoImage = list.row.find((f: ImageType) => f.width === 2 && f.height === 2);
        ...
        if (findTwoInTwoImage) {
            ...
            const oneInOneImages = list.row.filter((f: ImageType) => f.width !== 2 && f.height !== 2);
            ...
            return (
                <View style={{
                    ...
                    <View style={styles.oneInOneSingleImageWrapper}>
                        {oneInOneImages.map((_oneInOneItem: ImageType, _oneInOneItemIndex: number) => (
                            <Image
                                key={_oneInOneItemIndex}
                                source={{ uri: _oneInOneItem.path }}
                                style={[
                                    styles.oneInOneImageStyle,
                                    _oneInOneItemIndex / 2 === 0 && { marginBottom: 8 }
                                ]}
                            />
                        ))}
                    </View>
...
const styles = StyleSheet.create({
    oneInOneImageStyle: {
        width: '100%', height: 109, borderRadius: 8
    },
    oneInOneSingleImageWrapper: {
        width: '32%', marginBottom: 8
    },
})