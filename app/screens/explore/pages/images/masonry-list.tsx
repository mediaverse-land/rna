...
export function ImagePageMasonryList() {
    ...
    const renderMasonryList = masonryListImagesMockData.map((list: { row: Item[] }, index: number) => {
        const findTwoInTwoImage = list.row.find((f: Item) => f.width === 2 && f.height === 2);
        ...
        if (findTwoInTwoImage) {
            ...
            const oneInOneImages = list.row.filter((f: Item) => f.width !== 2 && f.height !== 2);
            ...
            return (
                <View style={{
                    ...
                    <View style={styles.oneInOneSingleImageWrapper}>
                        {oneInOneImages.map((_oneInOneItem: Item, _oneInOneItemIndex: number) => (
                            <Image
