...
export const MessageDetail = ({ route }: any) => {
  ...
  const getData = async () => {
    ...
    if (token !== null) {
      ...
      const { isSuccess, isError, res } = await getMessageDetailApiHandler(formattedToken, id);
            ...
            <Box width="100%" paddingRight={24} paddingLeft={24}>
              <Box direction="row" alignItems="center" justifyContent="space-between">
                <Box width="40%" height={20}>
                  <Text color="#fff" fontSize={16} fontWeight={400} lineHeight={20}>
                    {data.message}
                  </Text>
                </Box>
                <Box height={20}>
                  <Text color="#666680" fontSize={12} fontWeight={400} lineHeight={16}>
                    {date || ''}
                  </Text>
                </Box>
              </Box>
              <Box width="100%" marginTop={24}>
                <Text color="#666680" fontSize={16} fontWeight={400} lineHeight={16}>
                  {data.message}
                </Text>
              </Box>
            </Box>
