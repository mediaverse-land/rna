...
export function MessagesPage() {
  ...
  const getSessions = async () => {
    ...
    if (token !== null) {
      ...
      const { isSuccess, isError, res } = await getMessagesApiHandler(formattedToken);
  ...
  const renderMessageItem = ({ item }: { item: Message }) => {
    return (
      <TouchableOpacity activeOpacity={1} onPress={() => messageNavigateHandler(item.id)}>
        <Box
          width="100%"
          height={100}
          paddingTop={22}
  ...
  return (
    <SafeAreaView style={{ flex: 1 }}>
      <ScreenGradient>
        <Box width="100%">{pageHeaderComponent}</Box>
