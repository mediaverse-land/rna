...
export function CheckboxList({ labelText, optionsList }: Props) {
  ...
  function toggleSelectOptionHandler(option: SelectOption) {
    const existsInSelectedOptions = selectedOptions.find((d) => d.title === option.title);

    if (existsInSelectedOptions) {
      const filter = selectedOptions.filter((f) => f.title !== option.title);
  ...
  const renderOptions = optionsList
    ? optionsList.map((f: SelectOption) => {
        ...
        return (
          <Option onPress={() => toggleSelectOptionHandler(f)} key={f.id} color={active__color}>
            <RowAlignCenter>
              <Image source={{ uri: icon__path }} style={f.iconStyle} />
              <OptionText color={active__color}>{f.title}</OptionText>
            </RowAlignCenter>
            <View></View>
          </Option>
        );
